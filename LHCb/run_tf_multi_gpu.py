...
parser.add_argument( '--name', type=str, default="PPtR1R2Hits" )
...
parser.add_argument( '--trainwriteinterval', type=int, default="20" )
...
parser.add_argument( '--gpumergeinterval', type=int, default="10" )

parser.add_argument( '--inputvars', type=str, nargs='+',
                     default = [ 'TrackP', 'TrackPt', 'NumRich1Hits', 'NumRich2Hits' ] )
...
with tf.device('/cpu:0'):
  ...
  weight_col = 'Weight'
  readData[weight_col] = np.float32(1.0)
    ...
    for igpu in range(args.ngpus):
      ...
      with tf.device(tfdevice):
        with tf.name_scope( '%s_%d' % ("GPU",igpu) ) as scope:
          ...
          print( "Configuring GPU", igpu, "data sample size", data_train[igpu].shape[0] )
          ...
          with tf.name_scope("gradient_loss") :
            alpha             = tf.random_uniform(shape=[tf.shape(train_full)[0], 1], minval=0., maxval=1., seed = 5678+igpu )
...
def shareWeights( models ) :
    
  for iw in range(len(models[0].trainable_weights)) :

    # average value of tensor for this weight
    avg_w  = models[0].trainable_weights[iw].read_value()
    for ii in range(1,args.ngpus) :
      avg_w += models[ii].trainable_weights[iw].read_value()
    avg_w /= np.float32(args.ngpus)

    # set value in models to average
    for ii in range(0,args.ngpus) :
      models[ii].trainable_weights[iw].assign(avg_w)
...
with tf.Session(config=tf_config) as sess:
  ...
  for i in its :
    ...
    if i % args.gpumergeinterval == 0 :
        ...
        # merge the info from the GPUs...
        sess.graph._unsafe_unfinalize()
        shareWeights(gpu_critics)
        shareWeights(gpu_generators)
        sess.graph.finalize()
